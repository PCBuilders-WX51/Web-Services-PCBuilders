...
/*
    ...
    const myExperience: string = updateClientRequestDto.myExperience.trim();
    ...
    const nameGithub: string = updateClientRequestDto.nameGithub.trim();

    if (nameGithub.length <= 0) {
      notification.addError('Client nameGithub is required', null);
    }

    const imgClient: string = updateClientRequestDto.imgClient.trim();
