...
export class InitialSchema1638023897412 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        //await queryRunner.query(`CREATE TABLE \`clients\` (\`my_specialty\` varchar(150) NOT NULL, \`my_experience\` varchar(500) NOT NULL, \`description\` varchar(1000) NOT NULL, \`name_github\` varchar(150) NOT NULL, \`img_client\` varchar(500) NOT NULL, \`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`first_name\` varchar(30) NOT NULL, \`last_name\` varchar(30) NOT NULL, \`email\` varchar(150) NOT NULL, \`password\` varchar(15) NOT NULL, UNIQUE INDEX \`UQ_client\` (\`email\`, \`name_github\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`clients\` (\`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`first_name\` varchar(30) NOT NULL, \`last_name\` varchar(30) NOT NULL, \`email\` varchar(150) NOT NULL, \`password\` varchar(15) NOT NULL, UNIQUE INDEX \`UQ_client\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        //await queryRunner.query(`CREATE TABLE \`applications\` (\`clientId\` int UNSIGNED NOT NULL, \`announcementId\` int UNSIGNED NOT NULL, \`state\` enum ('P', 'A', 'D', 'N') NOT NULL DEFAULT 'P', \`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`date\` date NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
